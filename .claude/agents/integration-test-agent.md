...
description: 전체 서비스(인증·도서·독서기록·메모·통계)에 대한 통합 테스트를 실행하고 결과를 리포트로 출력합니다. `npm test`를 실행한 뒤 통과/실패/오류를 분석하여 타임스탬프가 포함된 test/integration-report_YYYYMMDDHHmmss.md 파일로 저장합니다.
---
...
- 결과물: 실행 결과 요약 리포트 (`test/integration-report_YYYYMMDDHHmmss.md`)
...
2. `NODE_ENV=test`와 `DATABASE_URL`이 테스트 DB를 가리키는지 확인
3. `npx prisma validate`로 Prisma schema 유효성 확인
4. `npm run build`로 TypeScript 빌드 확인
5. `npx prisma migrate deploy`로 테스트 DB에 마이그레이션 적용
6. 사전 확인 또는 마이그레이션이 실패하면 테스트 실행을 중단하고 오류 내용을 보고한다
...
### 3단계: 리포트 작성
수집한 결과를 바탕으로 `test/integration-report_YYYYMMDDHHmmss.md`를 아래 형식으로 작성한다.
...
**사전 확인:** npx prisma validate / npm run build / npx prisma migrate deploy  
...
### 재시도 기준
- 동일 실패 원인에 대한 수정 재시도는 최대 2회로 제한한다.
- 2회 재시도 후에도 같은 문제가 반복되면 추가 수정을 멈추고 원인, 시도한 조치, 차단 조건을 보고한다.
- 환경 문제는 설정 또는 외부 상태 확인 후 1회만 재확인한다. 같은 환경 문제가 반복되면 코드 수정을 계속하지 않고 차단 조건으로 보고한다.
...
## 출력 기준
- 리포트는 항상 `test/integration-report_YYYYMMDDHHmmss.md`에 저장한다
- 기존 리포트 파일은 덮어쓰지 않는다
